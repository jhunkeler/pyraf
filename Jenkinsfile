...
for (python_ver in matrix_python) {
    for (numpy_ver in matrix_numpy) {
        for (astropy_ver in matrix_astropy) {
            ...
            DEPS = "python=${python_ver} numpy=${numpy_ver} astropy=${astropy_ver}"
...
PIP_DEPS = ["git+https://github.com/numpy/numpy.git#egg=numpy --upgrade --no-deps",
            "git+https://github.com/astropy/astropy.git#egg=astropy --upgrade --no-deps",
            "git+https://github.com/spacetelescope/stsci.tools#egg=stsci.tools --upgrade --no-deps",
            "ipython --upgrade",
            "matplotlib --upgrade"]
...
for (python_ver in matrix_python) {
    ...
    install_pypi.build_mode = "install ${python_ver}-LATEST"

    install_pypi.build_cmds = ["${CONDA_INST} python=${python_ver} cython"]
    for (dep in PIP_DEPS) {
